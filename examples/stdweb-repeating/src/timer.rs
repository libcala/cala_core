...
use cala_core::os::web::{JsFn, JsVar, JsPromise};
...
impl JsTimer {
    pub fn new(res: i32, ms: i32) -> JsTimer {
        let reset_promise = unsafe {
            ...
            ").call(Some(&JsVar::from_i32(res)), Some(&JsVar::from_i32(ms))).unwrap().into_fn()
        };
        let promise = unsafe {
            reset_promise.call(None, None).unwrap().into_promise()
        };
        JsTimer {
...
impl Future for JsTimer {
    ...
    fn poll(mut self: Pin<&mut Self>, _cx: &mut Context) -> Poll<i32> {
        ...
        if let Poll::Ready(result) = this.promise.poll() {
            this.promise = unsafe { this.reset_promise.call(None, None).unwrap().into_promise() };
